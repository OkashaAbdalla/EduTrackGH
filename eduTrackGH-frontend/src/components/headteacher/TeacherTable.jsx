...
function TeacherActions({ teacher, readOnly, onToggleStatus, onViewDetails, onAssignClassroom, onDeleteTeacher, deletingId }) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      ...
      <button
        type="button"
        onClick={() => onAssignClassroom(teacher)}
    ...
    return (
      <Card>
        <div className="text-center py-10 px-6">
          ...
          {!readOnly && <Button variant="primary" onClick={onOpenCreate}>Create First Teacher</Button>}
        </div>
      </Card>
    );
  }
            ...
            <div key={id} className="p-3 rounded-lg border border-gray-200 dark:border-gray-700 space-y-2 min-w-0">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-semibold text-gray-900 dark:text-white break-words">{teacher.fullName}</p>
              ...
              <TeacherActions
                ...
                onAssignClassroom={onAssignClassroom}
      ...
      <div className="hidden sm:block overflow-x-auto table-scroll">
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-900">
            <tr>
              <th className="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Name</th>
              <th className="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Email</th>
              <th className="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Status</th>
              <th className="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Actions</th>
            </tr>
          </thead>
                ...
                <tr key={id} className="hover:bg-gray-50 dark:hover:bg-gray-800/70">
                  <td className="px-4 lg:px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">
                    <div className="flex items-center gap-3">
                      <ProfileAvatar src={teacher.avatarUrl} name={teacher.fullName} size="sm" />
                      <span className="break-words">{teacher.fullName}</span>
                    </div>
                  </td>
                  <td className="px-4 lg:px-6 py-4 text-sm text-gray-600 dark:text-gray-300 break-all">{teacher.email}</td>
