/**
 * Headteacher Notification Button — bell with parent alert sound
 */
...
const HeadteacherNotificationButton = () => {
  ...
  const fetchNotifications = useCallback(async (withSound = false) => {
    try {
      if (!withSound) setLoading(true);
      const response = await headteacherService.getNotifications();
      const notifs = response?.notifications || [];
      const latestUnread = response?.unreadCount || 0;
      const unreadNotifs = notifs.filter((n) => !n.read);
      setNotifications(unreadNotifs.slice(0, 8));
      ...
      console.error('Failed to fetch headteacher notifications:', error);
    } finally {
  ...
  useEffect(() => {
    ...
    socket.on('headteacher_notification', handler);
    socket.on('compliance_updated', handler);
    return () => {
      socket.off('headteacher_notification', handler);
            ...
            {loading ? (
              ...
              <div className="divide-y divide-[color:var(--glass-border)]">
                {notifications.map((notif) => (
                  <div
                    key={notif.id}
                    className="px-4 py-3 hover:bg-[color:var(--glass)] transition-colors bg-red-50/50 dark:bg-red-950/20"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-[color:var(--text-primary)]">
                          {notif.teacherName}:{' '}
                          <span className="text-red-600 dark:text-red-400 font-semibold">Unmarked</span>
                        </p>
                        {notif.date && (
                          <p className="text-xs text-[color:var(--text-secondary)] mt-0.5">
                            Attendance date: {notif.date}
                          </p>
                        )}
                        {notif.createdAt && (
              ...
              <div className="flex flex-col items-center justify-center py-8 px-4">
                ...
                <p className="text-xs text-[color:var(--text-secondary)] text-center">No unread notifications</p>
              </div>
            )}
          </div>
        </div>
...
export default HeadteacherNotificationButton;
